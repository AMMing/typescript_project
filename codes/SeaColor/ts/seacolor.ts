...
(() => {
    let path = '/template/SeaColor';
    let debug_path = 'http://ts.amoe.me/seacolor';
    let now_url = window.location.href.toLowerCase();
    let is_debug = now_url.indexOf('debug=ture') > 0;//手动进入调试页面
    AMing.Core.Helper.append_js(`${is_debug ? debug_path : path}/js/append.js`, () => (new SeaColor.Append(path)).init());
})();